...
packages <- c(
  ...
  "remotes", "AbiMatrix", "Cardinal", "EBImage", "reshape2", "IsoCorrectoR" # Additional packages
)
...
prepIsocoR <- function(measure_file = NA, molecule_file = NA, element_file = NA, outdir = ".") {
  ...
  # Rename row name
  #measure_file$`Measurements/Samples` <- gsub("Glutamate", "Glu", measure_file$`Measurements/Samples`)
  #measure_file$`Measurements/Samples` <- gsub("Glutamine", "Gln", measure_file$`Measurements/Samples`)
  ...
  metabolites <- c("Fumarate", "GABA", "Glutamate", "Glutamine", "Pyruvate", "Succinate")
  formulas <- c("C4H4O4LabC4", "C4H9N1O2LabC4", "C5H9N1O4LabC5", "C5H10N2O3LabC5", "C3H4O3LabC3", "C4H6O4LabC4")
  neutral_loss <- c(NA, NA, NA, NA, NA, NA)
...
IsoCorrection(
    ...
    FileOut = "corrected_data",         # Output file name
...
setwd("/Users/isaacmarin-valencia/Desktop/STAR_Protocol_Cereb/Excel_files")
#Load the library:
final_data <- read.csv("final_msi_data.csv") %>% 
              dplyr::select(-mz)

corrected_final_data <- read.csv("IsoCorrectoR_corrected_data_Corrected.csv")
...
```
>>>>>>> bf991022bfa5fa260477a59464424f744681a086
