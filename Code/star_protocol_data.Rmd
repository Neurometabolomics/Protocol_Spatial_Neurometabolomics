...
heatmap_data <- final_data_2 %>%
  ...
  pivot_wider(names_from = Tissue_Group, values_from = Avg_Normalized_Value, values_fill = 0) %>%
...
heatmap.2(
  heatmap_matrix,
  main = "Avg Normalized Value Heatmap",    # Title
  notecol = "black",                        # Cell label color
  ...
  labRow = rownames(heatmap_matrix),        # Use metabolite (ion) as row labels
