...
This is document describes the process of creating heatmap and enrichment plot for Figure 4E and 4F.
...
Here, we look at the ROIs. These ROIs were delineated using chemical segmentation, but it can also be manually drawn using Cardinal’s ROI tools if needed. The viewROIs function combines the logical images and displays them in a 2x2 grid using EBImage’s display() function for easy visualization and verification.
...
Once the ROIs are resized, they are reassigned to the MSI objects using the assignROI function, which maps each ROI to its corresponding dataset. This step guarantees that subsequent analyses focus only on the selected regions.
...
To evaluate the reproducibility of this method, we calculate the coefficient of variation (CV) across the ROIs from different biological samples. 
...
To account for natural isotope abundances, we use the IsocorrectoR package. This step prepares input files (measurement, molecule, and element) necessary for isotope correction. The prepIsocoR function formats the data for IsocorrectoR by renaming columns and rows in the measurement file, defining the isotopologue structure in the molecule file, and specifying isotope abundances and tracer properties in the element file. These files are then used with the IsoCorrection function to generate 13C corrected data. The results are saved in a specified output directory for downstream analysis.
...
After correcting for natural 13C abundance, we rename the output for traceability. From the corrected results, we prepare a heatmap and barplot to visualize the distribution and enrichment of metabolites across tissues.
...
Lastly, we validate the enrichment analysis from MSI data by comparing it with results obtained using the GC-MS method. The GC-MS measurements were performed using data from age-matched mice (P21, N=8), allowing us to cross-reference isotopologue distributions and ensure consistency between the two methods.
