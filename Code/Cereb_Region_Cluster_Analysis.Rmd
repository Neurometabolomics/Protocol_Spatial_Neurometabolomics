...
To ensure that all input and output files are correctly accessed and saved, we specify the working directory where our data is stored. For this analysis, the directory is set to Input/4C_cereb. However, your file path might be different, so make sure to set the working directory to the appropriate folder where your input files are stored. This folder should contain the data required for clustering and classification.
