---
title: "Cluster analysis for cereb region"
...
This is document to create cluster image on Figure 4C_cereb.
...
setwd("~/Documents/GitHub/STAR_Protocol/Input/4C_cereb")
...
At this stage, we save the final refined RefMet classification dataset as a CSV file, creating a clear checkpoint to preserve the data and prevent mix-ups if a different data matrix is analyzed in the future. Additionally, classification is added to new_tag.

```{r}
# Save the reference for future use
write.csv(new_refmet, file = "../../Results/4C_cereb/refmet_cereb.csv", row.names = FALSE)
```
...
new_refmet <- read.csv("../../Results/4C_cereb/refmet_cereb.csv")
...
ggsave(p1, filename = "../../Results/4C_cereb/4C_cereb.pdf", width = 7, height = 4)
