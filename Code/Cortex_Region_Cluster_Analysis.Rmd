...
To guarantee that all input and output files are correctly accessed and saved, we specify the working directory where the data is stored. For this analysis, the directory is set to Input/4C_cereb. However, since file paths may vary, it is important to set the working directory to the appropriate folder containing the input files required for clustering and classification.
...
The next step is to load the necessary data files, including the cluster data and the data matrix. The cluster files follow a sequential naming format (e.g., ClusterNo.1.csv, ClusterNo.2.csv, etc.), while the data matrix file is named Data_Matrix.csv. These files must be present in the working directory. Both file types are generated from IMAGEREVEAL, as detailed in manuscript part 6, step 10.
...
With updated labels, we re-run the classification step to achieve improved accuracy.
...
We manually address any residual gaps by filling in missing classifications directly. 
...
At this step, we save the final refined RefMet classification dataset as a CSV file, creating a clear checkpoint to preserve the data and prevent mix-ups if a different data matrix is analyzed in the future. Additionally, classification is added to new_tag.
...
Now that the classifications have been refined with RefMet, we integrate this information into the dataset. In this step, the Label column in new_tag is updated with the Standardized.name from the refined RefMet results, and hierarchical classifications—Superclass, Mainclass, and Subclass—are added. To maintain consistency, these classification columns are cleaned by removing unnecessary annotations (such as text in square brackets) and trimming whitespace. Finally, the enriched new_tag dataset is merged with the cluster data, creating a comprehensive dataset that combines refined metabolite information with cluster associations, preparing it for further analysis.
...
ggsave(p1, filename = "../../Results/4C_cortex/4C_cortex.pdf", width = 7, height = 4)
