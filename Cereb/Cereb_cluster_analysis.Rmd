...
# Convert colors into a named vector
color_map <- setNames(table1$Mainclass.color, table1$Mainclass)

# Create long-format data for barplot
long_barplot1 <- summarized_for_barplot |> # Your input data
  ...
  left_join(table1, by = "Mainclass") # Add colors based on `Mainclass`

# Plot with consistent colors
p1 <- ggplot(long_barplot1, aes(x = Cluster, y = Intensity, fill = Mainclass)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = color_map) + # Consistent color mapping
...
p1 <- ggplot(long_barplot1, aes(x = Cluster, y = Intensity, fill = Superclass)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = setNames(summarized_for_barplot$Superclass.color, 
                                      summarized_for_barplot$Superclass)) +
  labs(title = "Stacked Barplot: Superclass by Clusters",
       x = "Cluster",
       y = "Intensity",
       fill = "Super class") +
